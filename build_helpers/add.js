...
prompt.get(
    [
        {
            name: 'ruleName',
            description: 'What is the name of your eslint rule?\nFormat like no-fun-code\n',
            pattern: /^[a-z\-\d]*$/,
    ...
    function(err, result) {
        if (err) {
            ...
            fs.writeFileSync(
                ...
                fetchAndInsertVars(path.join(__dirname, 'templates/doc.md.template'), result)
            );
            fs.writeFileSync(
                path.join(__dirname, '../lib/rules/', result.ruleName + '.js'),
                fetchAndInsertVars(path.join(__dirname, 'templates/rule.js.template'), result)
            );
            fs.writeFileSync(
                path.join(__dirname, '../tests/lib/rules/', result.ruleName + '.js'),
                fetchAndInsertVars(path.join(__dirname, 'templates/test.js.template'), result)
            );
