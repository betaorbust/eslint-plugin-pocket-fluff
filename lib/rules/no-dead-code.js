 ...
 *
 * @param {object} comment   A block or line AST comment containing a replaceby annotation.
...
/**
 * Checks a comment for death and reports out if TODAY is after the replaceby.
 *
...
function checkForDeath(comment, context, today, matchDefs) {
    if (comment.value.match(matchDefs.annotation)) {
        ...
        let description;
        try {
            [, replaceby, user, description] = matchDefs.parse.exec(normalizedComment.value);
            replaceby = new Date(replaceby);
        } catch (e) {
            context.report({
        ...
        if (replaceby < today) {
            context.report({
                node: normalizedComment,
                data: { replaceby, user, description },
                messageId: 'deadCode'
            });
        }
    }
...
module.exports = {
    meta: {
        ...
        messages: {
            ...
            malformedAnnotation:
                '@replaceby annotation format: "@replaceby <m/d/yyyy> <email address> [<optional message>]"'
        },
        schema: [
            {
                type: 'object',
    ...
    create: function(context) {
        ...
        const ANNOTATION_MATCH_DEFAULT = '@replaceby|@deathdate'; //Deathdate was v1, so keep for backwards compat
