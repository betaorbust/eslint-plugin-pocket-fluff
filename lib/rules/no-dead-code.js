 ...
 * @param {date}   checkDate   The date to check deathdates against.
 * @returns {void}
 */
function checkForDeath(comment, context, checkDate) {
...
module.exports = {
    ...
    create: function(context) {
        const options = context.options[0] || {}; //values from the actual options

        const currentEpochTimeMS = options.currentEpochTimeMS
            ? Number.parseInt(options.currentEpochTimeMS, 10)
            : Date.now();
        const daysBeforeDeathToReport = options.daysBeforeDeathToReport
            ? Number.parseInt(options.daysBeforeDeathToReport, 10)
            : 0;

        const checkDate = new Date(currentEpochTimeMS + daysBeforeDeathToReport * ONE_DAY_IN_MS);

        return {
            Program: (node) => {
                if (node.comments && node.comments.length > 0) {
                    node.comments.forEach((comment) => {
                        checkForDeath(comment, context, checkDate);
                    });
                }
            }
        };
