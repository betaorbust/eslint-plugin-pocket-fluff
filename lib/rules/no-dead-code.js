...
function normalizeBlockAndLineComments(comment, matchDefs) {
	...
	const lineOffset = commentLines.findIndex((line) =>
		line.match(matchDefs.annotation)
	);
	...
	return {
		...
		loc: {
			start: { line: commentLine, column: comment.loc.start.column },
				...
				line: commentLine,
				column: comment.loc.start.column + value.length,
			},
		},
	};
}
...
function checkForDeath(comment, context, today, matchDefs) {
	if (comment.value.match(matchDefs.annotation)) {
		...
		let description;
		try {
			[, removeby, user, description] = matchDefs.parse.exec(
...
module.exports = {
	...
	create(context) {
		...
		const currentEpochTimeMS =
			Number.parseInt(ruleConfig.currentEpochTimeMS, 10) || Date.now();
		const daysBeforeToReport =
			Number.parseInt(ruleConfig.daysBeforeToReport, 10) || 0;
		const checkDate = new Date(
			currentEpochTimeMS + daysBeforeToReport * ONE_DAY_IN_MS
		);
