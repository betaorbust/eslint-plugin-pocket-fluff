 ...
 */
function isConst(name, varDef, constNameRegex) {
    ...
    if (!scope.functionExpressionScope) {
        variables.forEach(function(variable) {
            if (
                ...
                !variable.defs[0] ||
                !isConst(variable.name, variable.defs[0], constNameRegex)
            ) {
...
module.exports = {
    meta: {
        docs: {
            description: 'Disallow Reassignments of consts and Const-named variables.',
            category: 'Best Practices',
            recommended: true
        },
        messages: {
            reassignedConst: '{{name}} is a constant and should not be reassigned.'
        },
        schema: [
            {
                type: 'object',
